import { UserProfile, City, BuyRentResult, NewOldResult, ScoredCity, FinancialPoint } from '../types';
...
export function scoreBuyVsRent(profile: UserProfile): BuyRentResult {
  ...
  if (holdingPeriod >= 10) {
    horizonImpact = 20; horizonDesc = `${holdingPeriod} ans : largement favorable à l'achat`;
  } else if (holdingPeriod >= 7) {
    horizonImpact = 12; horizonDesc = `${holdingPeriod} ans : favorable à l'achat`;
  } else if (holdingPeriod >= 5) {
    horizonImpact = 5; horizonDesc = `${holdingPeriod} ans : légèrement favorable à l'achat`;
  } else if (holdingPeriod >= 3) {
    horizonImpact = -5; horizonDesc = `${holdingPeriod} ans : trop court pour rentabiliser l'achat`;
  } else {
    horizonImpact = -20; horizonDesc = `${holdingPeriod} ans : trop court, la location est préférable`;
  }
  score += horizonImpact;
  factors.push({ label: "Horizon de détention", impact: horizonImpact, description: horizonDesc });

  // 2. Stabilité professionnelle
  let stabImpact: number;
  let stabDesc: string;
  ...
  [stabImpact, stabDesc] = profMap[profile.profession] ?? [-10, 'Situation inconnue'];
  score += stabImpact;
  ...
  let rateDesc: string;
  if (rate < 2.5) {
    rateImpact = 15; rateDesc = `${rate} % : taux très attractif`;
  } else if (rate < 3.5) {
    rateImpact = 8; rateDesc = `${rate} % : taux correct`;
  } else if (rate < 4.5) {
    rateImpact = 0; rateDesc = `${rate} % : taux neutre`;
  } else {
    rateImpact = -10; rateDesc = `${rate} % : taux élevé, pèse sur la rentabilité`;
  }
  score += rateImpact;
  ...
  if (apportRatio >= 0.25) {
    apportImpact = 15; apportDesc = `Apport de ${Math.round(apportRatio * 100)} % : excellent`;
  } else if (apportRatio >= 0.15) {
    apportImpact = 10; apportDesc = `Apport de ${Math.round(apportRatio * 100)} % : bon`;
  } else if (apportRatio >= 0.1) {
    apportImpact = 5; apportDesc = `Apport de ${Math.round(apportRatio * 100)} % : minimum bancaire`;
  } else if (apportRatio >= 0.05) {
    apportImpact = -5; apportDesc = `Apport de ${Math.round(apportRatio * 100)} % : insuffisant pour beaucoup de banques`;
  } else {
    apportImpact = -15; apportDesc = `Apport inférieur à 5 % : accès au crédit compromis`;
  }
  score += apportImpact;
  factors.push({ label: "Apport personnel", impact: apportImpact, description: apportDesc });

  // 5. Mensualité vs loyer estimé (using median city pricing)
  ...
  // Estimate rent = price × yield/12 for an equivalent asset
  const estimatedRent = (profile.budget * 0.04) / 12; // ~4% yield estimate
  const ratio = estimatedRent > 0 ? monthly / estimatedRent : 1;
  let rentImpact: number;
  let rentDesc: string;
  if (ratio < 0.9) {
    rentImpact = 15; rentDesc = `Mensualité (${Math.round(monthly)} €) < loyer équivalent (${Math.round(estimatedRent)} €)`;
  } else if (ratio < 1.1) {
    rentImpact = 8; rentDesc = `Mensualité (${Math.round(monthly)} €) ≈ loyer équivalent`;
  } else if (ratio < 1.3) {
    rentImpact = 0; rentDesc = `Mensualité (${Math.round(monthly)} €) légèrement > loyer`;
  } else {
    rentImpact = -10; rentDesc = `Mensualité (${Math.round(monthly)} €) nettement > loyer équivalent (${Math.round(estimatedRent)} €)`;
  }
  score += rentImpact;
  ...
  return { score, recommendation, label, factors, estimatedMonthlyPayment: Math.round(monthly), estimatedMonthlyRent: Math.round(estimatedRent) };
}
...
export function scoreNewVsOld(profile: UserProfile): NewOldResult {
  ...
  // 1. Budget vs typical new-build prices
  ...
  if (budgetPerSqm >= 7000) {
    budgetImpact = 15; budgetDesc = `${Math.round(budgetPerSqm)} €/m² : budget compatible avec le neuf`;
  } else if (budgetPerSqm >= 5000) {
    budgetImpact = 5; budgetDesc = `${Math.round(budgetPerSqm)} €/m² : neuf accessible en petite couronne`;
  } else if (budgetPerSqm >= 3500) {
    budgetImpact = -5; budgetDesc = `${Math.round(budgetPerSqm)} €/m² : le neuf reste difficile — l'ancien offre plus de choix`;
  } else {
    budgetImpact = -15; budgetDesc = `${Math.round(budgetPerSqm)} €/m² : budget orienté vers l'ancien`;
  }
  score += budgetImpact;
  ...
  // 3. Tolérance aux travaux
  let travauxImpact: number;
  let travauxDesc: string;
  if (profile.riskTolerance === 'low') {
    travauxImpact = 15; travauxDesc = "Faible tolérance au risque : le neuf élimine les surprises de travaux";
  } else if (profile.riskTolerance === 'medium') {
    travauxImpact = 0; travauxDesc = "Tolérance modérée : les deux options sont envisageables";
  } else {
    travauxImpact = -15; travauxDesc = "Bonne tolérance au risque : l'ancien à rénover peut créer de la valeur";
  }
  score += travauxImpact;
  ...
  if (profile.desiredSurface <= 60) {
    surfaceImpact = 10; surfaceDesc = `${profile.desiredSurface} m² : les petites surfaces neuves sont plus accessibles`;
  } else if (profile.desiredSurface <= 90) {
    surfaceImpact = 0; surfaceDesc = `${profile.desiredSurface} m² : neutre`;
  } else {
    surfaceImpact = -15; surfaceDesc = `${profile.desiredSurface} m² : grandes surfaces neuves rares et chères en IDF`;
  }
  score += surfaceImpact;
  factors.push({ label: "Surface souhaitée", impact: surfaceImpact, description: surfaceDesc });

  // 5. Horizon (DPE / charges)
  let horizonImpact: number;
  let horizonDesc: string;
  if (profile.holdingPeriod >= 15) {
    horizonImpact = 10; horizonDesc = "Long terme : économies énergétiques du neuf se rentabilisent";
  } else if (profile.holdingPeriod >= 8) {
    horizonImpact = 5; horizonDesc = "Moyen terme : léger avantage du neuf sur les charges";
  } else {
    horizonImpact = -5; horizonDesc = "Court terme : surcoût du neuf difficile à amortir";
  }
  score += horizonImpact;
...
export function scoreCities(profile: UserProfile): ScoredCity[] {
  ...
  return IDF_CITIES
    .filter((city, idx, arr) => arr.findIndex(c => c.id === city.id) === idx) // deduplicate
    .map(city => {
      ...
      // 3. Transport (20%) — penalize if commute > maxCommute
      const commuteOk = city.transitTimeToCenter <= profile.maxCommute;
      const transportAdj = commuteOk
        ? city.transportScore * 10
        : Math.max(0, city.transportScore * 10 - (city.transitTimeToCenter - profile.maxCommute) * 2);

      // 4. Quality of life (15%)
      const qolAdj = city.qualityOfLife * 10;

      // 5. Market tension (10%) — low tension = easier to buy
      ...
      if (city.transitTimeToCenter > profile.maxCommute) cons.push(`Trajet vers Paris (${city.transitTimeToCenter} min) > votre limite`);
      ...
      if (city.hasRER) pros.push('Accès RER direct');
...
export function simulateFinancial(profile: UserProfile, city: City | null): FinancialPoint[] {
  ...
  const notaireRate = 0.075; // worst case ancien
  const upfrontCosts = downPayment + propertyValue0 * notaireRate;
  const annualCharges = pricePerSqm * profile.desiredSurface * 0.005; // ~0.5%/year
  const appreciation = 0.015; // 1.5%/yr
  const rentEquivalent = (propertyValue0 * 0.04) / 12; // 4% yield
  const rentInflation = 0.02;

  const points: FinancialPoint[] = [];
  let cumBuyCost = upfrontCosts;
  let cumRentCost = 0;
  let rentMonthly = rentEquivalent;

  for (let year = 0; year <= profile.holdingPeriod; year++) {
    if (year > 0) {
      const yearlyMortgage = monthly * 12;
      cumBuyCost += yearlyMortgage + annualCharges;
    ...
    const netWealthBuy = propertyValue - remainingLoan - (year === 0 ? 0 : 0);
